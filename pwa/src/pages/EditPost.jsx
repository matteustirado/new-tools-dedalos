...
import { ChevronLeft, MapPin, Zap, Send, Navigation } from 'lucide-react';
...
export default function EditPost() {
  ...
  const handlePublish = async () => {
    ...
    if (!academiaNome.trim()) {
      toast.warning('Por favor, informe onde você treinou hoje!');
      return;
    }
    ...
    <div className="min-h-screen bg-[#050505] text-white flex flex-col relative overflow-hidden">
      ...
      <main className="flex-1 p-4 max-w-md mx-auto w-full z-10 flex flex-col gap-6 animate-fade-in-up">
        <div className="w-full aspect-[4/5] bg-black rounded-3xl overflow-hidden border border-white/10 shadow-2xl relative group">
          ...
          <div className="absolute bottom-4 left-4 bg-black/60 backdrop-blur-md border border-white/10 px-3 py-1.5 rounded-full flex items-center gap-1.5">
            <Navigation
              size={12}
              className={gpsData ? 'text-emerald-400' : 'text-red-400'}
            />
            <span className="text-[10px] font-bold text-white/70 uppercase tracking-wider">
              {gpsData ? 'GPS Gravado' : 'Sem GPS'}
            </span>
          </div>
        </div>
