...
const TopCircle = ({ user, position }) => {
  ...
  return (
    <div className={`flex flex-col items-center gap-2 ${isFirst ? '-mt-6 z-10' : 'z-0'}`}>
      <Link
        to={profilePath}
        className={`relative rounded-full overflow-hidden ring-4 ${ringColor} ${sizeClass} bg-white/5 flex items-center justify-center shadow-2xl transition-transform hover:scale-105`}
      >
        {user?.foto_perfil ? (
          <img 
        ...
        <div className={`absolute -bottom-1 -right-1 w-6 h-6 rounded-full ${badgeColor} flex items-center justify-center text-[10px] font-black border-2 border-[#050505]`}>
          {position}
        </div>
      </Link>
      ...
      {user ? (
        <div className="flex flex-col items-center">
          <span className="text-white font-bold text-xs truncate w-20 text-center">
...
export default function Ranking() {
  ...
  const fetchRanking = async () => {
    try {
      const res = await axios.get(`${API_URL}/api/gym/rankings`);
  ...
  return (
    <div className="w-full relative overflow-x-hidden min-h-screen pb-24 flex flex-col bg-[#050505]">
      ...
      <div className="w-full flex-1 pt-6 px-4 pb-4">
        <h2 className="text-[10px] font-black text-white/40 uppercase tracking-[0.2em] mb-4 ml-2">
          Global
        </h2>
