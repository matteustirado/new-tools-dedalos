...
export default function EditPost() {
  ...
  useEffect(() => {
    ...
    if (isEditing) {
      ...
      toast.warning('Nenhum dado capturado.');
      navigate('/feed');
    }
  ...
  }, [navigate, photo, isEditing, existingPost, isRun, runData, runBackgroundImage]);
  ...
  useEffect(() => {
    const delayDebounceFn = setTimeout(async () => {
      if (searchQuery.length >= 2) {
        setIsSearching(true);
        try {
          const res = await axios.get(`${API_URL}/api/gym/users/search?q=${searchQuery}`);
  ...
  const handleAction = async () => {
    ...
    if (!academiaNome.trim()) { 
      toast.warning('Informe onde você treinou!'); 
      return; 
    }
    ...
    if (isDuo && !taggedUser && !isEditing) {
      toast.warning('Para um Treino em Dupla, você precisa marcar alguém!');
      return;
    }
    ...
    if (isEditing) {
      try {
        ...
        if (isArchived !== wasArchived) {
          ...
          await axios.put(`${API_URL}/api/gym/post/${existingPost.id}/${endpoint}`, { colaborador_cpf: user.cpf });
        }
      ...
      try {
        ...
        if (taggedUser) {
          formData.append('tagged_cpf', taggedUser.cpf);
        }
        ...
        if (isRun) {
          ...
          if (runBackgroundImage) {
            formData.append('foto_treino', dataURLtoFile(runBackgroundImage, 'run_background.jpg'));
          }
        } else {
        ...
        if (isArchived && res.data.checkin_id) {
          await axios.put(`${API_URL}/api/gym/post/${res.data.checkin_id}/archive`, { colaborador_cpf: user.cpf });
        }

        toast.success(`Check-in registrado com sucesso! 🍌`);
        isArchived ? navigate(`/${user.username}`) : navigate('/feed');
      } catch (err) {
      ...
      <main className="flex-1 p-4 max-w-md mx-auto w-full z-10 flex flex-col gap-6 animate-fade-in-up pb-8">
                  ...
                  <>
                    {isUpload ? (
                      <div className="absolute top-4 right-4 bg-black/60 backdrop-blur-md border border-orange-500/30 px-3 py-1.5 rounded-full flex items-center gap-1.5 shadow-[0_0_15px_rgba(249,115,22,0.3)]">
                        ...
                        <span className="text-xs font-black text-orange-400 tracking-wider">GALERIA (+0.5 PT)</span>
                      </div>
                    ) : (
                      <div className={`absolute top-4 right-4 bg-black/60 backdrop-blur-md border px-3 py-1.5 rounded-full flex items-center gap-1.5 shadow-[0_0_15px_rgba(16,185,129,0.3)] ${isDuo ? 'border-yellow-500/30' : 'border-emerald-500/30'}`}>
                        {isDuo ? <Users size={14} className="text-yellow-500" /> : <Zap size={14} className="text-emerald-400" />}
                        <span className={`text-xs font-black tracking-wider ${isDuo ? 'text-yellow-500' : 'text-emerald-400'}`}>
                          {isDuo ? 'DUPLA (+2 PTS)' : 'TEMPO REAL (+1 PT)'}
                        </span>
                      </div>
                    )}
                    ...
                    <div className={`absolute bottom-4 left-4 bg-black/70 backdrop-blur-md border ${localGpsData ? 'border-emerald-500/40' : 'border-red-500/40'} px-3 py-1.5 rounded-full flex items-center gap-1.5 shadow-lg transition-all`}>
                      ...
                      {localGpsData ? (
                        <span className="text-[10px] font-bold text-emerald-400 uppercase tracking-wider">GPS Gravado</span>
                      ) : (
        ...
        {isDuo && !isEditing && (
          <div className="flex flex-col gap-2 mt-2 relative z-50">
            <label className="text-xs font-bold text-yellow-500 ml-1 flex items-center gap-2 uppercase tracking-wider">
              <Users size={14} /> Quem treinou com você? <span className="text-red-500">*</span>
            </label>
            
            {taggedUser ? (
              <div className="flex items-center justify-between bg-yellow-500/10 border border-yellow-500/30 rounded-2xl p-3 animate-fade-in">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex flex-col">
                    ...
                    <span className="text-xs text-yellow-500 font-semibold">@{taggedUser.username}</span>
                  </div>
                </div>
        ...
        <div className="flex flex-col gap-2 mt-2">
          <label className="text-xs font-bold text-white/70 ml-1 uppercase tracking-wider">Privacidade</label>
        ...
        <div className="flex flex-col gap-2 mt-2">
          ...
          <input type="text" value={academiaNome} onChange={(e) => setAcademiaNome(e.target.value)} placeholder="Ex: Smart Fit - Paulista" className="w-full bg-white/5 border border-white/10 rounded-2xl p-4 text-white placeholder:text-white/30 focus:outline-none focus:border-yellow-500 focus:ring-1 focus:ring-yellow-500/50 transition-all font-bold" />
        </div>

        <div className="flex-1 flex flex-col">
          <label className="text-xs font-bold text-white/70 ml-1 mb-2 uppercase tracking-wider">Legenda (Opcional)</label>
          <textarea value={mensagem} onChange={(e) => setMensagem(e.target.value)} placeholder="Compartilhe como foi o seu treino!" className="w-full bg-white/5 border border-white/10 rounded-2xl p-4 text-white placeholder:text-white/30 focus:outline-none focus:border-yellow-500 focus:ring-1 focus:ring-yellow-500/50 transition-all resize-none h-24" />
        </div>
      </main>

      <div className="p-6 bg-gradient-to-t from-[#050505] via-[#050505] to-transparent z-10 pb-10 mt-auto">
        <button onClick={handleAction} disabled={loading} className="w-full max-w-md mx-auto bg-yellow-500 hover:bg-yellow-400 text-black font-black py-4 rounded-xl shadow-[0_10px_30px_rgba(234,179,8,0.2)] transition-all flex items-center justify-center gap-2 active:scale-95 disabled:opacity-70 disabled:active:scale-100">
          {loading ? <div className="w-6 h-6 border-2 border-black/30 border-t-black rounded-full animate-spin" /> : <>{isEditing ? <Save size={20} /> : <Send size={20} />}{isEditing ? 'SALVAR ALTERAÇÕES' : 'PUBLICAR CHECK-IN'}</>}
        </button>
      </div>
