...
export default function BottomNav() {
  ...
  const handleNavClick = (e, isReady, label, isActive) => {
    if (!isReady) {
      e.preventDefault();
      toast.info(`${label} em breve nas próximas atualizações! 🚧`);
      return;
    }
          ...
          return (
            <li key={item.id} className="relative flex flex-col items-center">
              ...
              >
                ...
                {isActive && (
                  <span className="absolute -bottom-1 w-1.5 h-1.5 bg-yellow-400 rounded-full shadow-[0_0_8px_rgba(250,204,21,1)] animate-fade-in" />
                )}
              </Link>
            </li>
          );
