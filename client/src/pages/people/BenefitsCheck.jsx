...
export default function BenefitsCheck() {
    ...
    const handleSearch = (e) => {
        ...
        if (search === '123') {
            setEmployee({
                ...
                role: 'Gerente',
                benefits: {
                    consumo: { status: 'available', limit: 100, used: 20 },
                    entrada: { status: 'used', date: '13/01/2026 14:30' }
                }
            });
        } else {
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-slate-900 to-slate-800 flex">
            <Sidebar activePage="benefits" headerTitle="Gestão de Pessoas" headerIcon="groups" group="people" />

            <main className="ml-64 flex-1 p-8">
                <header className="mb-8">
                    <h1 className="text-3xl font-bold text-white mb-2">Conferência de Benefícios</h1>
                    <p className="text-white/50">Verifique a disponibilidade de benefícios diários.</p>
                </header>
                ...
                <div className="max-w-2xl mx-auto">
                    ...
                    <form onSubmit={handleSearch} className="relative mb-10">
                        ...
                        />
                        <button type="submit" className="absolute right-3 top-3 bg-blue-600 p-2 rounded-xl text-white hover:bg-blue-500 transition-colors">
                        ...
                        <div className="liquid-glass rounded-3xl p-8 animate-fade-in">
                            <div className="flex items-center gap-6 mb-8 border-b border-white/10 pb-6">
                                ...
                                <div className="ml-auto">
                                    <span className="bg-green-500/20 text-green-400 px-4 py-1 rounded-full text-sm font-bold uppercase tracking-widest border border-green-500/30">Ativo</span>
                                </div>
                            </div>
                                ...
                                <div className="bg-white/5 p-5 rounded-xl border border-white/5 flex justify-between items-center">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-sm text-white/50">Limite: R$ {employee.benefits.consumo.limit},00</p>
                                        </div>
                                    </div>
                                    <div className="text-right">
                                        <p className="text-2xl font-bold text-white">R$ {employee.benefits.consumo.used},00</p>
                                ...
                                <div className={`p-5 rounded-xl border flex justify-between items-center ${employee.benefits.entrada.status === 'used' ? 'bg-red-500/10 border-red-500/30' : 'bg-green-500/10 border-green-500/30'}`}>
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-sm text-white/50">{employee.benefits.entrada.status === 'used' ? 'Já utilizado hoje' : 'Disponível'}</p>
                                        </div>
                                    </div>
                                    {employee.benefits.entrada.status === 'used' && (
                                        <div className="text-right">
                                            <span className="text-xs text-red-300 bg-red-900/40 px-2 py-1 rounded">Utilizado às {employee.benefits.entrada.date.split(' ')[1]}</span>
                                        </div>
                                    )}
                                </div>
                            </div>
