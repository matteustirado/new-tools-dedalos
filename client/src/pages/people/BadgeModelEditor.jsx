        ...
        <div className="w-1/3 bg-[#1a1a1a] rounded-2xl p-6 border border-white/10 flex flex-col h-full shadow-2xl">
          ...
          <div className="overflow-y-auto custom-scrollbar pr-2 flex-1 space-y-6 pb-4">
              ...
              {!isCreating ? (
                <div className="flex gap-2">
                  <select 
                    ...
                    className="flex-1 bg-black/30 border border-white/10 rounded-lg p-2 text-white font-bold text-sm"
                  >
                    {templates.map(t => (
                      <option key={t.id} value={t.role_name}>{t.role_name}</option>
                    ))}
                  </select>
                  ...
                  {selectedRole !== 'PADRAO' && (
                    <button 
                      ...
                      className="bg-red-600 hover:bg-red-500 p-2 rounded text-white"
                    >
                ...
                <div className="flex gap-2">
                  <input 
                    ...
                    placeholder="NOME..." 
                    className="flex-1 bg-black/30 border border-white/10 rounded-lg p-2 text-white" 
                  />
            ...
            <div className="bg-white/5 p-4 rounded-xl border border-white/5">
              ...
              <div className="mb-4">
                <div className="flex justify-between mb-1">
                  ...
                  <span className="text-xs text-orange-400 font-bold">{config.headerHeight}%</span>
                </div>
                ...
                <input 
                  ...
                  onChange={e => setConfig({...config, headerHeight: parseInt(e.target.value)})} 
              ...
              <div className="mb-4">
                ...
                <div className="flex gap-2 items-center">
                  <div className="w-10 h-10 bg-black/50 border border-white/10 rounded flex items-center justify-center overflow-hidden flex-shrink-0">
                    {config.logoUrl ? (
                      <img 
                        src={config.logoUrl.startsWith('http') ? config.logoUrl : `${API_URL}${config.logoUrl}`} 
                        alt="" 
                  ...
                  <button 
                    ...
                    className="flex-1 bg-white/10 hover:bg-white/20 border border-white/20 rounded py-2 text-[10px] uppercase font-bold text-white transition-colors"
                  >
                    Trocar Imagem
                  </button>
              ...
              <div>
                <div className="flex justify-between mb-1">
                  <label className="text-xs text-white/70">Tam. Logo</label>
                  <span className="text-xs text-orange-400 font-bold">{config.logoSize || 80}%</span>
                </div>
              ...
              <div className="space-y-2">
                <label className="flex items-center gap-3 cursor-pointer group select-none">
                  ...
                  <input 
                    type="checkbox" 
                    className="hidden" 
                    checked={config.textures?.includes('marca_dagua') || false} 
                    onChange={() => toggleTexture('marca_dagua')} 
                  />
                </label>
                ...
                <label className="flex items-center gap-3 cursor-pointer group select-none">
                  ...
                  <input 
                    type="checkbox" 
                    className="hidden" 
                    checked={config.textures?.includes('pontilhado') || false} 
                    onChange={() => toggleTexture('pontilhado')} 
                  />
                </label>
                ...
                <label className="flex items-center gap-3 cursor-pointer group select-none">
                  ...
                  <input 
                    type="checkbox" 
                    className="hidden" 
                    checked={config.textures?.includes('direcionais') || false} 
                    onChange={() => toggleTexture('direcionais')} 
                  />
                </label>
              </div>
            </div>
            ...
            <div className="bg-white/5 p-4 rounded-xl border border-white/5">
                ...
                <div className="flex gap-2">
                  {['circle', 'rounded', 'square'].map(shape => (
                    <button 
                      ...
                      className={`flex-1 py-2 text-[10px] uppercase font-bold border border-white/10 rounded ${config.photoShape === shape ? 'bg-blue-600 text-white border-blue-500' : 'bg-white/5 text-white/50'}`}
                    >
                      {shape}
                    </button>
                  ))}
              ...
              <div className="mb-2">
                <div className="flex justify-between mb-1">
                  <label className="text-xs text-white/70">Pos. Foto</label>
                  <span className="text-xs text-blue-400 font-bold">{config.photoY || 0}px</span>
                </div>
                
                <input 
                  type="range" 
                  min="-100" max="100" step="2" 
                  value={config.photoY || 0} 
                  onChange={e => setConfig({...config, photoY: parseInt(e.target.value)})} 
              ...
              <div>
                <div className="flex justify-between mb-1">
                  <label className="text-xs text-white/70">Pos. Texto</label>
                  <span className="text-xs text-blue-400 font-bold">{config.contentY}px</span>
                </div>
                
                <input 
                  type="range" 
                  min="-80" max="80" step="2" 
                  value={config.contentY} 
                  onChange={e => setConfig({...config, contentY: parseInt(e.target.value)})} 
                  className="w-full accent-blue-500 h-1 bg-white/20 rounded-lg appearance-none cursor-pointer" 
                />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-xs text-white/50 block mb-1">Tam. Nome</label>
                
                <input 
                  type="number" 
                  value={config.nameSize} 
                  onChange={e => setConfig({...config, nameSize: parseInt(e.target.value)})} 
                  className="w-full bg-black/30 border border-white/10 rounded-lg p-2 text-white text-sm" 
                />
              </div>
              
              <div>
                <label className="text-xs text-white/50 block mb-1">Tam. Cargo</label>
                
                <input 
                  type="number" 
                  value={config.roleSize} 
                  onChange={e => setConfig({...config, roleSize: parseInt(e.target.value)})} 
                  className="w-full bg-black/30 border border-white/10 rounded-lg p-2 text-white text-sm" 
                />
              </div>
            </div>
